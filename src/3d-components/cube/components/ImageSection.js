...
const ImageSection = ({ imageList }) => {
  ...
  const { rows, separation, scale, leftPadding, topPadding, items, webViewer, imageViewer } =
    imageList;
  ...
  return (
    <group>
      ...
      {floatingCamera && imageViewer && displayImage && (
        <Html
          as='div'
          transform
          rotation={imageViewer.rotation}
          position={imageViewer.position}
          zIndexRange={[0, 10]}
          style={{
        ...
        >
          ...
          > 
            <img
              ...
              onClick={() => {
                window.open("pictures/resumes/my-resume.pdf");
              }}
              src={displayImage}
            />
          </div>
